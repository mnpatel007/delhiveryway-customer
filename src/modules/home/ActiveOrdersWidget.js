  ...
  const fetchActiveOrders = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const activeOrdersFiltered = ordersArray.filter(
        ...
        sortedOrders.forEach((order) => {
          console.log(
            `Order ${order.orderNumber}: Status = ${order.status}, Shop = ${order.shopId?.name}, InquiryTime = ${order.shopId?.inquiryAvailableTime}`
          );
        });
          ...
          const updatedOrders = prevOrders.map((order) => {
            if (order._id === data.orderId) {
              ...
              if (data.estimatedDeliveryTime)
                updatedOrder.estimatedDeliveryTime = data.estimatedDeliveryTime;
    ...
    return (
      <div className="active-orders-widget error-state">
        <div className="widget-header">
          ...
          <button className="refresh-btn" onClick={fetchActiveOrders} title="Try again">
            <svg viewBox="0 0 24 24">
    ...
    <div className="active-orders-widget">
      <div className="widget-header">
        ...
        <div className="header-actions">
          ...
          <button className="refresh-btn" onClick={fetchActiveOrders} title="Refresh orders">
            <svg viewBox="0 0 24 24">
              <path d="M21 12a9 9 0 11-3-6.7M21 4v4h-4" />
            </svg>
          </button>
                    ...
                    <div className="order-details">
                      ...
                      {order.shopId && order.shopId.name && (
                        <span className="order-shop">
                          <svg viewBox="0 0 24 24" className="os-ic">
